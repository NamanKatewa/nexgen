  ...
  return (
    <section className="w-full py-16 md:py-24">
      <div className="container mx-auto px-4 md:px-6">
        <motion.div {...animationProps} className="mb-16 text-center">
        ...
        <div className="mb-20 flex flex-col gap-8 md:flex-row md:justify-center">
          {contactInfo.map(({ icon: Icon, text }, index) => (
            <motion.div
              key={text}
              {...animationProps}
              transition={{ ...animationProps.transition, delay: index * 0.1 }}
              className="group flex flex-col items-center text-center md:w-1/2"
            >
              <div className="mb-4 rounded-full bg-blue-100/20 p-4 shadow-lg transition-all duration-300 group-hover:bg-blue-200">
                <Icon className="h-8 w-8 text-primary transition-colors duration-300 group-hover:text-primary/80" />
              </div>
              <p className="w-3/4 text-lg font-medium text-blue-950">{text}</p>
            </motion.div>
          ))}
        </div>
